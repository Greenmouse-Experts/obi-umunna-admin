...
const Testimonial = () => {
  const { data, isLoading, refetch } = useGetHook(`admin/testimony/fetch`);
  ...
  return (
    <>
      ...
      <Modal title={"Add Testimonial"}>
        <AddTestimonial close={() => setShowModal(false)} refetch={refetch} />
      </Modal>
      <Edit title={selected?.name}>
        <EditTestimonial item={selected} close={() => showEdit(false)} refetch={refetch} />
      </Edit>
      <Delete title="" noHead>
        <ReusableModal
