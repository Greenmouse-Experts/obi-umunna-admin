...
import SponsorModal from "../../admin/sponsors/SponsorModal";
import { formatAsNgnMoney } from "../../services/helpers";

const Sponsors = () => {
  const { data, isLoading: loading, refetch } = useGetHook("admin/donors");
  ...
  const handleStatus = () => {
      ...
      handlePost(`admin/action/donor`, payload, "application/json", onStatus);
    }
  ...
  return (
    <div className="px-5">
                    ...
                    {!!items.length &&
                      items?.map((item, i) => (
                        <tr key={i}>
                          ...
                          <td className="align-middle fs-500 whitespace-nowrap px-6 lg:px-10 py-4 text-left border-b border-[#CECECE]">
                        <span className={`px-2 py-1 rounded-md text-white  ${item.publish !== 0 ? "bg-green-600" :" bg-red-600"}`}>
                        {item.publish === 0
                                  ? "Unpublished"
                                  : "Published"}
                        </span>
      ...
      {showDetails && (
        <SponsorModal item={selected} close={() => setShowDetails(false)} />
      )}
      ...
      <Status title={""} noHead>
        <ReusableModal
          title={`Are you sure you want to ${status  === 0
            ? "Publish"
            : "Unpublish"} Sponsor?`}
          cancelTitle="No, cancel"
          actionTitle={`Yes,  ${status  === 0
            ? "Publish"
            : "Unpublish"}`}
